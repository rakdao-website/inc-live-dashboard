...
import { ToolTimeoutError, UserError } from "./errors.mjs";
...
import { isAsyncStandardSchemaValidationError, isStandardSchemaWithJSON, } from "./utils/standardSchema.mjs";
...
export const FUNCTION_TOOL_INVALID_OUTPUT_FAILURE_CALLBACK = Symbol('openai.agents.functionToolInvalidOutputFailureCallback');
        ...
        if (timeoutErrorFunction) {
            try {
                ...
                if (invalidOutputFailure) {
                    details?.[FUNCTION_TOOL_INVALID_OUTPUT_FAILURE_CALLBACK]?.(invalidOutputFailure);
...
export function tool(options) {
    ...
    function validateOutput(output, runContext, details) {
        ...
        try {
            ...
            throw createInvalidToolOutputError(`Invalid output for function tool '${name}'.`, error, { runContext, output, details });
        }
    }
    ...
    async function resolveToolFailure(runContext, error, details) {
        ...
        const redactedBeforeCallback = redactedInputBeforeCallback || redactedOutputBeforeCallback;
        const callbackError = invalidInputFailure?.error ?? invalidOutputFailure?.error ?? error;
        ...
        try {
            ...
            const callbackInvalidOutputFailure = getInvalidToolOutputFailure(callbackFailure);
            if (callbackInvalidOutputFailure) {
                details?.[FUNCTION_TOOL_INVALID_OUTPUT_FAILURE_CALLBACK]?.(callbackInvalidOutputFailure);
    ...
    functionToolInputParsers.set(invoke, {
        ...
        validationMode: !isZodObject(options.parameters) &&
