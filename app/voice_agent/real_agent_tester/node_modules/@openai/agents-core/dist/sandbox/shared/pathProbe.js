...
exports.probeSandboxDirectoryExists = probeSandboxDirectoryExists;
...
async function probeSandboxDirectoryExists(options) {
    if (!(await probeSandboxPathExists(options))) {
        return false;
    }
    const result = await options.runCommand(`test -d ${(0, shell_1.shellQuote)(options.path)} && test -x ${(0, shell_1.shellQuote)(options.path)}`);
    if (result.status === 0 && !hasProcessFailure(result)) {
        return true;
    }
    if (result.status === 1 &&
