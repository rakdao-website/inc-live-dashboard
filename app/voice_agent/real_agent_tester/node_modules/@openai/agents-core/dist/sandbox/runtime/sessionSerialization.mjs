import { assertSandboxSessionStateUsable, withExclusiveSandboxStateInspection, } from "../mountSecurity.mjs";
import { SANDBOX_SESSION_STATE_VERSION, } from "../session.mjs";
import { getPreviousSerializedSessionsByAgent, toSessionStateEnvelope, } from "./sessionState.mjs";
import { withExclusiveProvidedSessionManifestMutation } from "./providedSessionManifest.mjs";
export async function serializeSandboxRuntimeState(args) {
    ...
    for (const [currentAgentKey, session] of sessionsByAgentKey.entries()) {
        ...
        const serializedEntry = await withExclusiveProvidedSessionManifestMutation(session.state, async () => await withExclusiveSandboxStateInspection(session.state, async () => {
