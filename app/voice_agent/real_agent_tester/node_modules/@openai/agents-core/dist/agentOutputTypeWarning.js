...
exports.hasDefinitelyDifferentOutputTypes = hasDefinitelyDifferentOutputTypes;
const tools_1 = require("./utils/tools.js");
const NOT_COMPARABLE = Symbol('notComparable');
function isPrimitiveOutputType(value) {
...
function getComparableJsonSchemaOutputType(outputType) {
...
function getComparableOutputType(outputType) {
    ...
    if (outputTypeDiscriminator.found &&
        outputTypeDiscriminator.value === 'json_schema') {
        return getComparableJsonSchemaOutputType(outputType);
    }
    try {
        const serializedOutputType = (0, tools_1.convertAgentOutputTypeToSerializable)(outputType);
        if (typeof serializedOutputType !== 'object' ||
...
function toComparableJsonValue(value, ancestors) {
    if (value === null ||
        typeof value === 'string' ||
    ...
    try {
        if (isArray) {
            ...
            for (let index = 0; index < length; index += 1) {
                ...
                const comparableElement = toComparableJsonValue(element.value, ancestors);
        ...
        const comparable = Object.create(null);
        for (const key of Object.keys(descriptors).sort()) {
            ...
            const comparableProperty = toComparableJsonValue(descriptor.value, ancestors);
...
function stringifyComparableJsonValue(value) {
    if (value === null) {
        return 'null';
    }
    ...
    return `{${Object.keys(value)
        .map((key) => `${JSON.stringify(key)}:${stringifyComparableJsonValue(value[key])}`)
        .join(',')}}`;
}
function getOutputTypeFingerprint(outputType) {
    const comparableOutputType = getComparableOutputType(outputType);
    if (comparableOutputType === undefined) {
        return undefined;
    }
    const comparableJsonValue = toComparableJsonValue(comparableOutputType, new Set());
 ...
 * @internal
 */
function hasDefinitelyDifferentOutputTypes(outputTypes) {
    ...
    for (let leftIndex = 0; leftIndex < outputTypes.length - 1; leftIndex += 1) {
        for (let rightIndex = leftIndex + 1; rightIndex < outputTypes.length; rightIndex += 1) {
            ...
            if (isPrimitiveOutputType(leftOutputType) ||
...
//# sourceMappingURL=agentOutputTypeWarning.js.map