...
import { getFunctionToolLookupKey, getFunctionToolLegacyStateKeyFromStateKey, getFunctionToolStateKeyForCall, getHostedMcpApprovalRequestIdentity, getHostedMcpApprovalIdentityFromStateKey, getHostedMcpApprovalStateKey, } from "./toolIdentity.mjs";
import { Usage } from "./usage.mjs";
import { getToolInvocationCallId, getToolInvocationFingerprint, } from "./toolInvocation.mjs";
...
function cloneApprovalMap(records) {
    return new Map([...records].map(([toolName, record]) => [
        toolName,
        cloneApprovalRecord(record),
    ]));
}
function cloneAgentApprovalMap(records) {
    return new Map([...records].map(([agent, approvals]) => [
        agent,
...
export class RunContext {
     ...
     * @internal
     */
    _cloneForRunStateDeserialization() {
        ...
        clone.#functionApprovalState = {
            approvalsByAgent: cloneAgentApprovalMap(this.#functionApprovalState.approvalsByAgent),
            legacyApprovals: cloneApprovalMap(this.#functionApprovalState.legacyApprovals),
        };
        clone.#approvalInvocations = new Map([...this.#approvalInvocations].map(([agent, invocations]) => [
            agent,
            new Map(invocations),
        ]));
        clone.#toolApprovalsByAgent = cloneAgentApprovalMap(this.#toolApprovalsByAgent);
        return clone;
    }
    /**
    ...
    _replaceApprovalState(source) {
        ...
        for (const [agent, approvals] of source.#functionApprovalState
            .approvalsByAgent) {
            this.#functionApprovalState.approvalsByAgent.set(agent, cloneApprovalMap(approvals));
        }
        this.#functionApprovalState.legacyApprovals.clear();
        for (const [toolName, approval] of source.#functionApprovalState
            .legacyApprovals) {
            this.#functionApprovalState.legacyApprovals.set(toolName, cloneApprovalRecord(approval));
        }
     ...
     * @internal
     */
    _rebuildApprovalInvocations(invocations, agentsByIdentity) {
        this.#approvalInvocations = new Map();
        this.#toolApprovalsByAgent = new Map();
     ...
     * @internal
     */
    _mergeApprovalInvocations(invocations, agentsByIdentity) {
        this._validateApprovalInvocations(invocations, agentsByIdentity);
        for (const { agentIdentity, invocations: byCallId, approvals, } of invocations) {
            const agent = agentsByIdentity.get(agentIdentity);
            this.#mergeAgentApprovalInvocations(agent, new Map(Object.entries(byCallId)));
     ...
     * @internal
     */
    _validateApprovalInvocations(invocations, agentsByIdentity) {
        const seenAgentIdentities = new Set();
        for (const { agentIdentity, invocations: byCallId } of invocations) {
            if (!agentsByIdentity.has(agentIdentity) ||
                seenAgentIdentities.has(agentIdentity)) {
                throw new UserError('RunState contains invalid approval invocation ownership for the current agent graph.');
            }
     ...
     * @internal
     */
    _bindLegacyApprovalInvocation(approvalItem) {
        ...
        if (decision !== undefined) {
            agentInvocations.set(callId, getToolInvocationFingerprint(toolName, approvalItem.rawItem));
     ...
     * @internal
     */
    _validateAgentApprovalInvocation(agent, callId, fingerprint) {
        const approvedFingerprint = this.#approvalInvocations
            .get(agent)
            ?.get(callId);
        if (approvedFingerprint !== undefined &&
            approvedFingerprint !== fingerprint) {
            throw new ModelBehaviorError(`Tool call ID ${callId} was reused for a different invocation after an approval decision.`);
        }
    }
    /**
     ...
     * @internal
     */
    _validateToolInvocation(agent, toolName, rawItem) {
        const callId = getToolInvocationCallId(rawItem);
        if (!callId) {
            throw new ModelBehaviorError('Tool invocation is missing a non-empty call ID.');
        }
     ...
     * @internal
     */
    _resolveToolInvocationApproval(agent, toolName, rawItem) {
        const { callId, fingerprint } = this._validateToolInvocation(agent, toolName, rawItem);
        if (rawItem.type === 'function_call') {
            return this.#resolveFunctionApprovalEntryGroups(agent, toolName, callId, this.#approvalInvocations.get(agent)?.get(callId) === fingerprint)?.decision;
        }
     ...
     * @internal
     */
    _getToolInvocationRejectionMessage(agent, toolName, rawItem) {
        if (rawItem.type === 'function_call') {
            const { callId, fingerprint } = this._validateToolInvocation(agent, toolName, rawItem);
            const resolution = this.#resolveFunctionApprovalEntryGroups(agent, toolName, callId, this.#approvalInvocations.get(agent)?.get(callId) === fingerprint);
            if (resolution?.decision !== false) {
                return undefined;
            }
            return this.#getRejectionMessageForRejectedCall(resolution.entries, callId);
        }
        const decision = this._resolveToolInvocationApproval(agent, toolName, rawItem);
        if (decision !== false) {
            return undefined;
        }
        const callId = getToolInvocationCallId(rawItem);
        ...
        const stickyEntries = getHostedMcpApprovalRequestIdentity(rawItem)
            ? this.#getHostedMcpApprovalEntries(toolName)
            : this.#getApprovalEntries(toolName, false);
        return this.#getRejectionMessageForRejectedCall([...entries, ...stickyEntries], callId);
    }
    /**
    ...
    /** @internal */
    _rebuildHostedMcpApprovals(approvals) {
        this.#hostedMcpApprovals = new Map();
        this._mergeHostedMcpApprovals(approvals);
    }
    /** @internal */
    _mergeHostedMcpApprovals(approvals) {
        for (const [stateKey, incoming] of Object.entries(approvals)) {
            ...
            this.#hostedMcpApprovals.set(stateKey, current ? mergeApprovalRecords(current, incoming) : incoming);
        }
    }
    /**
    ...
    _getFunctionRejectionMessage(toolName, callId, agent) {
        const resolution = this.#resolveFunctionApprovalEntryGroups(agent, toolName, callId);
        if (resolution?.decision !== false) {
            return undefined;
        }
        return this.#getRejectionMessageForRejectedCall(resolution.entries, callId);
    ...
    #serialize(agentIdentityKeys) {
        ...
        if (!agentIdentityKeys) {
            for (const [stateKey, incoming] of this.#hostedMcpApprovals) {
                ...
                approvals.set(identity.toolName, current ? mergeApprovalRecords(current, incoming) : incoming);
            }
        ...
        if (agentIdentityKeys) {
            ...
            for (const agent of approvalAgents) {
                ...
                const approvalsByTool = this.#toolApprovalsByAgent.get(agent) ?? new Map();
                const agentIdentity = agentIdentityKeys.get(agent);
                if (agentIdentity &&
    ...
    isToolApproved(approval) {
        ...
        if (agent) {
            return this.#resolveFunctionApprovalEntryGroups(agent, toolName, callId, true, functionTool)?.decision;
        }
        const includeFunctionState = functionTool ||
            getFunctionToolLegacyStateKeyFromStateKey(toolName) !== undefined;
    ...
    /** @internal */
    _getHostedMcpApprovalStatus(approvalItem) {
        ...
        return this.#resolveApprovalEntries(this.#getHostedMcpApprovalEntries(stateKey), identity.requestId);
    }
    /** @internal */
    _getHostedMcpRejectionMessage(approvalItem) {
        const identity = getHostedMcpApprovalRequestIdentity(approvalItem);
        const stateKey = identity
            ? getHostedMcpApprovalStateKey(identity)
            : undefined;
        ...
        return this.#getRejectionMessageFromEntries(this.#getHostedMcpApprovalEntries(stateKey), identity.requestId);
    }
    #resolveApprovalEntries(approvalEntries, callId) {
        return (this.#resolveExactApprovalEntries(approvalEntries, callId) ??
            this.#resolveStickyApprovalEntries(approvalEntries));
    }
    #resolveExactApprovalEntries(approvalEntries, callId) {
    ...
    #resolveApprovalEntryGroups(approvalEntryGroups, callId, includeExactDecisions = true) {
    ...
    #resolveFunctionApprovalEntryGroups(agent, toolName, callId, includeExactDecisions = true, includeFunctionAliases = false) {
        return this.#resolveApprovalEntryGroups([
            this.#getFunctionApprovalEntries(toolName, includeFunctionAliases, agent),
            this.#getLegacyFunctionApprovalEntries(toolName),
        ], callId, includeExactDecisions);
    }
    #resolveStickyApprovalEntries(approvalEntries) {
    ...
    #getRejectionMessageForRejectedCall(approvalEntries, callId) {
    ...
    approveTool(approvalItem, { alwaysApprove = false } = {}) {
        const toolName = this.#getApprovalItemStorageKey(approvalItem, alwaysApprove);
        ...
        const approvals = isFunctionCall
            ...
            : getHostedMcpApprovalRequestIdentity(approvalItem)
                ...
                : this.#approvals;
        const approvalKey = this.#getApprovalStorageKey(toolName, isFunctionCall, approvals);
        ...
        if (!isFunctionCall) {
            ...
            const scopedKey = this.#getApprovalStorageKey(toolName, false, scopedApprovals);
    ...
    rejectTool(approvalItem, { alwaysReject = false, message, } = {}) {
        const toolName = this.#getApprovalItemStorageKey(approvalItem, alwaysReject);
        const isFunctionCall = approvalItem.rawItem.type === 'function_call';
        const approvals = isFunctionCall
            ? this.#getFunctionApprovalMap(approvalItem.agent)
            : getHostedMcpApprovalRequestIdentity(approvalItem)
        ...
        if (!isFunctionCall) {
            const scopedApprovals = this.#getToolApprovalMap(approvalItem.agent);
            const scopedKey = this.#getApprovalStorageKey(toolName, false, scopedApprovals);
            this.#recordPerCallApproval(scopedApprovals, scopedKey, callId, false, message);
        }
    }
    #recordPerCallApproval(approvals, approvalKey, callId, approved, message) {
    ...
    #bindApprovalInvocation(toolName, approvalItem) {
        const { callId, fingerprint } = this._validateToolInvocation(approvalItem.agent, toolName, approvalItem.rawItem);
        this.#getApprovalInvocations(approvalItem.agent).set(callId, fingerprint);
    }
    #getApprovalInvocations(agent) {
    ...
    #getToolApprovalEntries(toolName, agent) {
    ...
    #getHostedMcpApprovalEntries(stateKey) {
        const approval = this.#hostedMcpApprovals.get(stateKey);
        return approval ? [approval] : [];
    }
    #getToolApprovalMap(agent) {
    ...
    #copyPerCallToolApproval(agent, toolName, callId) {
        const sourceEntries = this.#getApprovalEntries(toolName, false);
        const approved = sourceEntries.some((entry) => Array.isArray(entry.approved) && entry.approved.includes(callId));
        const rejected = sourceEntries.some((entry) => Array.isArray(entry.rejected) && entry.rejected.includes(callId));
        ...
        if (rejected) {
            this.#recordPerCallApproval(approvals, approvalKey, callId, false, this.#getRejectionMessageFromEntries(sourceEntries, callId));
        }
    }
    ...
    #getApprovalItemStorageKey(approvalItem, persistent) {
        const hostedIdentity = getHostedMcpApprovalRequestIdentity(approvalItem);
        if (!hostedIdentity) {
            if (approvalItem.rawItem.type === 'hosted_tool_call' && persistent) {
                throw new UserError('Persistent hosted approval decisions require valid MCP approval request provider data.');
            }
            return this.#getApprovalItemToolName(approvalItem);
        }
        if (!hostedIdentity.requestId) {
            throw new UserError('Hosted MCP approval decisions require a non-empty request id.');
        }
        if (!hostedIdentity.serverLabel || !hostedIdentity.toolName) {
            throw new UserError('Hosted MCP approval decisions require a non-empty server label and tool name.');
        }
    ...
    #setToolApprovalRecord(agent, toolName, incoming) {
