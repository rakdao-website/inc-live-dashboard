 ...
 */
export async function resolveInterruptedTurn(agent, originalInput, originalPreStepItems, newResponse, processedResponse, runner, state, toolErrorFormatter, agentToolParentRunConfig, signal, validateHandoffAgent) {
...
export async function resolveTurnAfterModelResponse(agent, originalInput, originalPreStepItems, newResponse, processedResponse, runner, state, toolErrorFormatter, agentToolParentRunConfig, errorHandlers, signal, preflightedToolCalls, attemptedErrors, validateHandoffAgent) {
    ...
    if (handoffRuns.length > 0) {
        if (signal?.aborted) {
            ...
            return await executeHandoffCalls(agent, originalInput, preStepItems, newItems, newResponse, handoffRuns, runner, state._context, getRunStateTurnSpanParent(state) ?? state._currentAgentSpan, validateHandoffAgent);
        }
    }
    const completedStep = await maybeCompleteTurnFromToolResults({
        agent,
