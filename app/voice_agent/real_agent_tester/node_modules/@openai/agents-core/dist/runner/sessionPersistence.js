...
async function addSessionItems(session, items, runContext) {
    if (runContext && (0, session_1.isRunContextAwareSession)(session)) {
...
async function saveStreamInputToSession(session, sessionInputItems, runContext) {
    if (!session) {
        return;
    }
...
async function prepareInputItemsWithSession(input, session, sessionInputCallback, options, runContext) {
    ...
    const history = (0, items_1.trimToLatestCompaction)(await getSessionItems(session, runContext));
...
async function runCompactionOnSession(session, responseId, state, compactionMode) {
    ...
    const compactionResult = (0, session_1.isRunContextAwareSession)(session)
