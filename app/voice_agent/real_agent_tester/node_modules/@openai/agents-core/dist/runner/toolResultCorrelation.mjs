import { getToolSearchExecution, getToolSearchMatchKey, getToolSearchProviderCallId, } from "../tooling.mjs";
...
export function getSimpleToolResultTypeForCall(type) {
    return typeof type === 'string'
        ? SIMPLE_TOOL_RESULT_TYPE_BY_CALL_TYPE[type]
        : undefined;
}
export function isSimpleToolResultType(type) {
    return typeof type === 'string' && SIMPLE_TOOL_RESULT_TYPES.has(type);
}
export function getToolResultCorrelationForCall(item) {
...
export function getToolResultCorrelationForResult(item) {
    if (!item || typeof item !== 'object') {
        return undefined;
    }
    const type = item.type;
    if (type === TOOL_SEARCH_OUTPUT) {
        ...
        return createCorrelation(TOOL_SEARCH_OUTPUT, getToolSearchProviderCallId(item));
    }
    const callId = item.callId;
...
export function getToolResultCorrelationKey(correlation) {
...
export function getUnresolvedToolResultCorrelationsForResponse(items) {
    const calls = [];
    const results = [];
...
//# sourceMappingURL=toolResultCorrelation.mjs.map