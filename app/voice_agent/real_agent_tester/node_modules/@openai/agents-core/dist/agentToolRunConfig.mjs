...
import { mergeTracingConfig } from "./tracing/config.mjs";
...
export function setToolCallParentSpanOnDetails(details, parent) {
    if (!parent) {
        return;
    }
...
export function getToolCallParentSpanFromDetails(details) {
    if (!details || typeof details !== 'object') {
        return undefined;
    }
...
export function mergeAgentToolRunConfig(inheritedRunConfig, toolRunConfigOverride) {
    ...
    const mergedTracing = mergeTracingConfig(inheritedRunConfig.tracing, toolRunConfigOverride.tracing);
