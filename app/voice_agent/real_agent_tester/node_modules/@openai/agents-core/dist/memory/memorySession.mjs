...
export class MemorySession {
    ...
    logger;
    items;
    appliedHistoryTransactions = new Map();
    constructor(options = {}) {
    ...
    async applyHistoryTransaction(args) {
        ...
        if (existing) {
            if (!structurallyEqual(existing, transaction)) {
                throw new UserError('Session history operation was already applied with a different transaction.');
            }
            return;
        }
        let nextItems;
...
function snapshotHistoryTransactionArgs(args) {
    let operationId;
    let transaction;
    try {
    ...
    if (typeof operationId !== 'string' || !operationId.trim()) {
        throw new UserError('Session history transaction operationId must be a non-empty string.');
    }
...
function snapshotHistoryTransaction(transaction) {
    let snapshot;
    try {
        ...
        throw new UserError('Session history transaction items contain invalid or unsupported data.');
    }
    ...
    if (record.type === 'append_items') {
        if (Object.keys(record).length !== 2 ||
            ...
            throw new UserError('Session history append transaction fields are invalid.');
        }
        try {
            return {
                type: record.type,
    ...
    if (Object.keys(record).length !== 3 ||
        ...
        throw new UserError('Session history suffix transaction fields are invalid.');
    }
    try {
        return {
            type: record.type,
...
function snapshotAgentItems(items) {
    const snapshots = snapshotSupportedValue(items);
...
function trySnapshotAgentItems(items) {
    try {
...
function snapshotSupportedValue(value, ancestors = new WeakSet()) {
    if (value === null ||
        value === undefined ||
        typeof value === 'string' ||
    ...
    if (value instanceof Uint8Array) {
        ...
        if (keys.length !== value.length ||
            keys.some((key) => typeof key !== 'string' || !isArrayIndexForLength(key, value.length))) {
...
function isArrayIndexForLength(key, length) {
    if (!/^(0|[1-9]\d*)$/.test(key)) {
        return false;
    }
