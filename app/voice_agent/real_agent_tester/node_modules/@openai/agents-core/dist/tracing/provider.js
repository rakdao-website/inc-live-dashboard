...
function resolveTracingIdGenerator(idGenerator) {
    return {
        generateTraceId: idGenerator?.generateTraceId ?? utils_1.defaultTracingIdGenerator.generateTraceId,
        generateSpanId: idGenerator?.generateSpanId ?? utils_1.defaultTracingIdGenerator.generateSpanId,
    };
}
class TraceProvider {
    #multiProcessor;
    ...
    async dispatchTrace(trace) {
        if (this.#disabled) {
            if (logger_1.default.dontLogModelData || logger_1.default.dontLogToolData) {
                logger_1.default.debug('Tracing is disabled. Not dispatching trace.');
            }
            else {
                logger_1.default.debug('Tracing is disabled, Not dispatching trace %o', trace);
            }
            return;
        }
    ...
    async dispatchSpan(span) {
        if (this.#disabled) {
            if (logger_1.default.dontLogModelData || logger_1.default.dontLogToolData) {
                logger_1.default.debug('Tracing is disabled. Not dispatching span.');
            }
            else {
                logger_1.default.debug('Tracing is disabled, Not dispatching span %o', span);
            }
            return;
        }
    ...
    async dispatchSpanStart(span) {
        if (this.#disabled) {
            if (logger_1.default.dontLogModelData || logger_1.default.dontLogToolData) {
                logger_1.default.debug('Tracing is disabled. Not dispatching span start.');
            }
            else {
                logger_1.default.debug('Tracing is disabled, Not dispatching span start %o', span);
            }
            return;
        }
    ...
    async dispatchSpanEnd(span) {
        if (this.#disabled) {
            if (logger_1.default.dontLogModelData || logger_1.default.dontLogToolData) {
                logger_1.default.debug('Tracing is disabled. Not dispatching span end.');
            }
            else {
                logger_1.default.debug('Tracing is disabled, Not dispatching span end %o', span);
            }
            return;
        }
    ...
    createTrace(traceOptions) {
        if (this.#disabled) {
            if (logger_1.default.dontLogModelData || logger_1.default.dontLogToolData) {
                logger_1.default.debug('Tracing is disabled. Not creating trace.');
            }
            else {
        ...
        if (logger_1.default.dontLogModelData || logger_1.default.dontLogToolData) {
            logger_1.default.debug('Creating trace. Trace data is redacted.');
        }
        else {
    ...
    createSpan(spanOptions, parent) {
        if (this.#disabled || spanOptions.disabled) {
            if (logger_1.default.dontLogModelData || logger_1.default.dontLogToolData) {
                logger_1.default.debug('Tracing is disabled. Not creating span.');
            }
            else {
        ...
        if (spanOptions.spanId === utils_1.NOOP_TRACE_OR_SPAN_ID) {
            logger_1.default.debug('Span id is no-op, returning NoopSpan');
            return new spans_1.NoopSpan(spanOptions.data, this.#multiProcessor);
        }
        let parentId;
        let traceId;
        ...
        if (!parent) {
            ...
            if (!currentTrace) {
                logger_1.default.debug('No active trace. Make sure to start a trace with `withTrace()` first. Returning NoopSpan.');
                return new spans_1.NoopSpan(spanOptions.data, this.#multiProcessor);
            }
            if (currentSpan instanceof spans_1.NoopSpan ||
                currentSpan?.spanId === utils_1.NOOP_TRACE_OR_SPAN_ID ||
                currentTrace instanceof traces_1.NoopTrace ||
                currentTrace.traceId === utils_1.NOOP_TRACE_OR_SPAN_ID) {
            ...
            if (parent instanceof traces_1.NoopTrace ||
                parent.traceId === utils_1.NOOP_TRACE_OR_SPAN_ID) {
            ...
            if (parent instanceof spans_1.NoopSpan ||
                parent.spanId === utils_1.NOOP_TRACE_OR_SPAN_ID ||
                parent.traceId === utils_1.NOOP_TRACE_OR_SPAN_ID) {
        ...
        if (!traceId || traceId === utils_1.NOOP_TRACE_OR_SPAN_ID) {
        ...
        if (spanId === utils_1.NOOP_TRACE_OR_SPAN_ID) {
            logger_1.default.debug('Span id is no-op, returning NoopSpan');
            return new spans_1.NoopSpan(spanOptions.data, this.#multiProcessor);
        }
        if (logger_1.default.dontLogModelData || logger_1.default.dontLogToolData) {
            logger_1.default.debug('Creating span. Span data is redacted.');
        }
        else {
            logger_1.default.debug(`Creating span ${JSON.stringify(spanOptions.data)} with id ${spanId}`);
        }
        ...
        if (!this.#shutdownPromise) {
            this.#shutdownPromise = (async () => {
                try {
                    ...
                    (0, logger_1.logModelAndToolActionError)(logger_1.default, 'Error shutting down tracing provider', error);
                }
            })();
        }
    ...
    #addCleanupListeners() {
        if ((0, _shims_1.supportsProcessLifecycleEvents)() &&
