...
const JSON_SCHEMA_DEPTH_ERROR = 'JSON schema is too deeply nested to process safely. Simplify or flatten the schema, or disable strict mode.';
class JsonSchemaDepthError extends UserError {
}
function assertSafeJsonSchemaDepth(depth) {
    if (depth > MAX_JSON_SCHEMA_DEPTH) {
        throw new JsonSchemaDepthError(JSON_SCHEMA_DEPTH_ERROR);
    }
}
function createJsonSchemaTraversalState() {
    return {
        activeSchemas: new WeakSet(),
        visitedDepths: new WeakMap(),
    };
}
function enterJsonSchemaTraversal(schema, state, depth) {
...
function leaveJsonSchemaTraversal(schema, state) {
    state.activeSchemas.delete(schema);
}
function assertJsonSchemaDepth(schema) {
    assertJsonSchemaPhysicalDepth(schema);
}
function assertJsonSchemaPhysicalDepth(schema) {
    const pending = [];
    const visitedDepths = new WeakMap();
...
export function isJsonSchemaDepthError(error) {
...
export function prepareOpenAIStrictToolSchema(schema) {
    return prepareOpenAIStrictToolSchemaInternal(schema, true);
}
export function assertOpenAIStrictToolSchemaPreservesOpenObjects(schema) {
    assertJsonSchemaDepth(schema);
    validateOpenAIStrictJsonSchema(schema, schema, createJsonSchemaTraversalState(), true, true);
}
function prepareOpenAIStrictToolSchemaInternal(schema, requireUnambiguousNormalization) {
    assertJsonSchemaDepth(schema);
    validateOpenAIStrictProviderSchema(schema, schema);
    validateOpenAIStrictJsonSchema(schema, schema, createJsonSchemaTraversalState(), requireUnambiguousNormalization);
    ...
    const strictSchema = ensureStrictSchemaEntry(structuredClone(schema), schema, context, true);
    stabilizeLocalJsonSchemaReferences(strictSchema, context);
    context.preparedRoot = strictSchema;
    return {
        schema: strictSchema,
        normalizeInput: (value) => normalizeStrictJsonSchemaValue(strictSchema, value, context),
    };
}
function ensureStrictSchemaEntry(entry, originalEntry, context, isRoot = false) {
    ...
    if (schemaConvertsObjectProperties(record) && properties !== undefined) {
        ...
        const originalRequired = new Set(Array.isArray(originalRecord?.required)
    ...
    for (const key of ['$defs', 'definitions']) {
        ...
        if (typeof nested === 'object' &&
            ...
            for (const [nestedKey, nestedValue] of Object.entries(nested)) {
                nested[nestedKey] =
                    ensureStrictSchemaEntry(nestedValue, originalNested?.[nestedKey] ?? nestedValue, context);
            }
        }
    }
    for (const key of ['anyOf']) {
        ...
        if (Array.isArray(nested)) {
            record[key] = nested.map((value, index) => ensureStrictSchemaEntry(value, originalNested[index] ?? value, context));
        }
    }
    ...
    if (Array.isArray(items)) {
        const originalItemEntries = Array.isArray(originalItems)
            ? originalItems
            : [];
        record.items = items.map((value, index) => ensureStrictSchemaEntry(value, originalItemEntries[index] ?? value, context));
    }
    else if (typeof items === 'object' && items !== null) {
        record.items = ensureStrictSchemaEntry(items, originalItems ?? items, context);
    }
...
function schemaConvertsObjectProperties(schema) {
    return (isRecord(schema.properties) &&
        (schema.type === 'object' ||
            (Array.isArray(schema.type) && schema.type.includes('object')) ||
            typeof schema.type === 'undefined'));
}
function stabilizeLocalJsonSchemaReferences(preparedRoot, context) {
    ...
    const hoistedReferences = new WeakMap();
    ...
    for (let index = 0; index < context.references.length; index += 1) {
        ...
        const originalTarget = resolveLocalJsonSchemaReference(originalReference, context.originalRoot);
        if (typeof originalTarget === 'undefined') {
            continue;
        }
        const preparedTarget = isRecord(originalTarget)
            ? (context.preparedSchemas.get(originalTarget) ??
                ensureStrictSchemaEntry(structuredClone(originalTarget), originalTarget, context))
            : originalTarget;
        const pointerTarget = resolveLocalJsonSchemaReference(originalReference, preparedRoot);
        ...
        if (!definitionName) {
            ...
            if (typeof existingDefinitions !== 'undefined' &&
                !isRecord(existingDefinitions)) {
                throw new UserError('Cannot stabilize local JSON schema references because the root `$defs` value is not an object. Use an object-valued `$defs` or disable strict mode.');
            }
            ...
            } while (Object.prototype.hasOwnProperty.call(definitions, definitionName));
...
function wrapNullableSchema(schema, syntheticNullableSchemas) {
    if (schema === false) {
...
export function stripStrictNullsForJsonSchema(schema, value) {
    if (!isRecord(schema)) {
        return value;
    }
    return prepareOpenAIStrictToolSchema(schema).normalizeInput(value);
}
function normalizeStrictJsonSchemaValue(schema, value, context) {
    if (value === undefined) {
        return undefined;
    }
    let currentSchema = schema;
    if (isRecord(currentSchema)) {
        const syntheticNullableSchema = context.syntheticNullableSchemas.get(currentSchema);
    ...
    if (Array.isArray(value)) {
        ...
        if (Array.isArray(items)) {
            return value.map((entry, index) => normalizeStrictJsonSchemaValue(items[index], entry, context));
        }
        if (items && typeof items === 'object') {
            return value.map((entry) => normalizeStrictJsonSchemaValue(items, entry, context));
        }
        return value;
    }
    if (!isRecord(value) ||
    ...
    for (const [key, propertySchema] of Object.entries(properties)) {
        const nextValue = normalizeStrictJsonSchemaValue(propertySchema, normalized[key], context);
...
function validateOpenAIStrictProviderSchema(schema, rootSchema, state = createJsonSchemaTraversalState(), depth = 1) {
    if (typeof schema === 'boolean') {
        assertSafeJsonSchemaDepth(depth);
        return;
    }
    if (!isRecord(schema)) {
        throw new UserError('Cannot convert a JSON schema containing a non-boolean, non-object schema node to strict mode. Replace the invalid node with a JSON schema object or boolean, or disable strict mode.');
    }
    if (!enterJsonSchemaTraversal(schema, state, depth)) {
        return;
    }
    if (schema !== rootSchema && '$id' in schema) {
        throw new UserError('Cannot convert a JSON schema with a nested `$id` resource to strict mode because local references are resolved only within the root resource. Remove the nested `$id` or disable strict mode.');
    }
    for (const keyword of unsupportedStrictNormalizationKeywords) {
        if (keyword in schema) {
            throw new UserError(`Cannot convert a JSON schema using unsupported keyword \`${keyword}\` to strict mode. Remove the keyword or disable strict mode.`);
        }
    }
    if ('$ref' in schema) {
        if (typeof schema.$ref !== 'string') {
            throw new UserError('Cannot convert a JSON schema with a non-string `$ref` to strict mode.');
        }
        const resolved = resolveLocalJsonSchemaReference(schema.$ref, rootSchema);
        if (typeof resolved === 'undefined') {
            throw new UserError(`Cannot convert unresolved or external JSON schema reference \`${schema.$ref}\` to strict mode. Use a local reference or disable strict mode.`);
        }
        validateOpenAIStrictProviderSchema(resolved, rootSchema, state, depth + 1);
    }
    if ('anyOf' in schema) {
        if (!Array.isArray(schema.anyOf) || schema.anyOf.length === 0) {
            throw new UserError('Cannot convert a JSON schema with an invalid `anyOf` to strict mode.');
        }
    ...
    if (properties) {
        for (const propertySchema of Object.values(properties)) {
            validateOpenAIStrictProviderSchema(propertySchema, rootSchema, state, depth + 1);
        }
    }
    ...
    if ('additionalProperties' in schema) {
        validateOpenAIStrictProviderSchema(schema.additionalProperties, rootSchema, state, depth + 1);
    }
    for (const key of ['$defs', 'definitions']) {
        ...
        for (const definition of Object.values(definitions)) {
            validateOpenAIStrictProviderSchema(definition, rootSchema, state, depth + 1);
        }
    }
    leaveJsonSchemaTraversal(schema, state);
}
function validateOpenAIStrictJsonSchema(schema, rootSchema, state = createJsonSchemaTraversalState(), requireUnambiguousNormalization = true, rejectOpenObjects = false, depth = 1) {
    if (typeof schema === 'boolean') {
        assertSafeJsonSchemaDepth(depth);
        return;
    }
    ...
    if ('$ref' in schema) {
        ...
        const unsupportedSibling = Object.keys(schema).find((key) => !referenceMetadataKeywords.has(key));
        if (unsupportedSibling) {
            throw new UserError(`Cannot convert a JSON schema combining \`$ref\` with sibling keyword \`${unsupportedSibling}\` to strict mode. Move the constraint into the referenced schema or disable strict mode.`);
        }
        const resolved = resolveLocalJsonSchemaReference(schema.$ref, rootSchema);
        if (typeof resolved === 'undefined') {
            throw new UserError(`Cannot convert unresolved or external JSON schema reference \`${schema.$ref}\` to strict mode. Use a local reference or disable strict mode.`);
        }
        validateOpenAIStrictJsonSchema(resolved, rootSchema, state, requireUnambiguousNormalization, rejectOpenObjects, depth + 1);
    }
    if ('anyOf' in schema) {
        if (!Array.isArray(schema.anyOf) || schema.anyOf.length === 0) {
            throw new UserError('Cannot convert a JSON schema with an invalid `anyOf` to strict mode.');
        }
        const unsupportedSibling = Object.keys(schema).find((key) => !anyOfMetadataKeywords.has(key));
        if (unsupportedSibling) {
            throw new UserError(`Cannot convert a JSON schema combining \`anyOf\` with sibling keyword \`${unsupportedSibling}\` to strict mode. Move the constraint into each branch or disable strict mode.`);
        }
        for (const branch of schema.anyOf) {
            validateOpenAIStrictJsonSchema(branch, rootSchema, state, requireUnambiguousNormalization, rejectOpenObjects, depth + 1);
        }
        ...
        if (requireUnambiguousNormalization &&
            schema.anyOf.some((branch) => jsonSchemaRequiresSyntheticNullStripping(branch, rootSchema, createJsonSchemaTraversalState(), depth + 1))) {
            throw new UserError('Cannot convert an `anyOf` branch containing optional object properties to strict mode because plain JSON Schema tools do not select a branch during input normalization. Use a Zod schema, make the branch properties required, or disable strict mode.');
        }
    }
    const properties = isRecord(schema.properties)
        ? schema.properties
    ...
    const hasObjectType = schema.type === 'object' ||
        (Array.isArray(schema.type) && schema.type.includes('object'));
    const hasDeclaredProperties = properties !== undefined && Object.keys(properties).length > 0;
    if (rejectOpenObjects &&
        hasObjectType &&
        schema.additionalProperties !== false &&
        (!hasDeclaredProperties || 'additionalProperties' in schema)) {
        throw new UserError('Cannot convert an open JSON schema object to strict mode without changing its accepted values. Set `additionalProperties: false`, declare at least one property, or disable strict mode.');
    }
    if (schemaConvertsObjectProperties(schema) && properties) {
        const required = new Set(Array.isArray(schema.required) ? schema.required.map(String) : []);
        for (const [key, propertySchema] of Object.entries(properties)) {
            validateOpenAIStrictJsonSchema(propertySchema, rootSchema, state, requireUnambiguousNormalization, rejectOpenObjects, depth + 1);
            if (!required.has(key)) {
    ...
    if (Array.isArray(items)) {
        for (const item of items) {
            validateOpenAIStrictJsonSchema(item, rootSchema, state, requireUnambiguousNormalization, rejectOpenObjects, depth + 1);
        }
    }
    else if (typeof items !== 'undefined') {
        validateOpenAIStrictJsonSchema(items, rootSchema, state, requireUnambiguousNormalization, rejectOpenObjects, depth + 1);
    }
    for (const key of ['$defs', 'definitions']) {
        const definitions = schema[key];
        if (!isRecord(definitions)) {
        ...
        for (const definition of Object.values(definitions)) {
            validateOpenAIStrictJsonSchema(definition, rootSchema, state, requireUnambiguousNormalization, rejectOpenObjects, depth + 1);
        }
    }
    leaveJsonSchemaTraversal(schema, state);
}
function getKnownJsonSchemaNullability(schema, rootSchema, depth = 1) {
    const nullability = getJsonSchemaNullability(schema, rootSchema, new Set(), depth);
    if (nullability === 'unknown') {
        throw new UserError('Cannot determine whether an optional JSON schema property accepts `null`. Make its nullability explicit with a supported schema form, make the property required, or disable strict mode.');
    }
    return nullability;
}
function getJsonSchemaNullability(schema, rootSchema, visitedReferences = new Set(), depth = 1) {
    assertSafeJsonSchemaDepth(depth);
    if (schema === true) {
        return 'allows';
    }
    ...
    if ('$ref' in schema) {
        const referencedNullability = getReferencedJsonSchemaNullability(schema.$ref, rootSchema, visitedReferences, depth + 1);
    ...
    if ('anyOf' in schema) {
        ...
        const entries = schema.anyOf.map((entry) => getJsonSchemaNullability(entry, rootSchema, visitedReferences, depth + 1));
...
function jsonSchemaRequiresSyntheticNullStripping(schema, rootSchema, state = createJsonSchemaTraversalState(), depth = 1) {
    if (!isRecord(schema) || !enterJsonSchemaTraversal(schema, state, depth)) {
        return false;
    }
    try {
        if (typeof schema.$ref === 'string') {
            const resolved = resolveLocalJsonSchemaReference(schema.$ref, rootSchema);
            return jsonSchemaRequiresSyntheticNullStripping(resolved, rootSchema, state, depth + 1);
        }
        if (Array.isArray(schema.anyOf)) {
            return schema.anyOf.some((branch) => jsonSchemaRequiresSyntheticNullStripping(branch, rootSchema, state, depth + 1));
        }
        const properties = schemaConvertsObjectProperties(schema) && isRecord(schema.properties)
            ? schema.properties
            : {};
        const required = new Set(Array.isArray(schema.required) ? schema.required.map(String) : []);
        for (const [key, propertySchema] of Object.entries(properties)) {
            if ((!required.has(key) &&
                getKnownJsonSchemaNullability(propertySchema, rootSchema, depth + 1) === 'disallows') ||
                jsonSchemaRequiresSyntheticNullStripping(propertySchema, rootSchema, state, depth + 1)) {
                return true;
            }
        }
        const items = schema.items;
        return Array.isArray(items)
            ? items.some((item) => jsonSchemaRequiresSyntheticNullStripping(item, rootSchema, state, depth + 1))
            : jsonSchemaRequiresSyntheticNullStripping(items, rootSchema, state, depth + 1);
    }
    finally {
        leaveJsonSchemaTraversal(schema, state);
    }
}
function getReferencedJsonSchemaNullability(reference, rootSchema, visitedReferences, depth) {
    ...
    return getJsonSchemaNullability(resolved, rootSchema, new Set([...visitedReferences, reference]), depth);
}
function resolveLocalJsonSchema(schema, context) {
    let current = schema;
    ...
    while (isRecord(current) && typeof current.$ref === 'string') {
        ...
        const resolved = resolveLocalJsonSchemaReference(reference, context.preparedRoot);
        if (typeof resolved === 'undefined') {
            return schema;
        }
        current = resolved;
...
function resolveLocalJsonSchemaReference(reference, rootSchema) {
