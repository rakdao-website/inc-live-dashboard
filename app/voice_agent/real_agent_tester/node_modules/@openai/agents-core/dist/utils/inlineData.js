"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.getInlineMediaType = getInlineMediaType;
exports.formatInlineData = formatInlineData;
const base64_1 = require("./base64.js");
function getInlineMediaType(value) {
    if (typeof value.mediaType === 'string' && value.mediaType.length > 0) {
        return value.mediaType;
    }
    if (typeof value.mimeType === 'string' && value.mimeType.length > 0) {
        return value.mimeType;
    }
    return undefined;
}
function formatInlineData(data, mediaType) {
    if (typeof data === 'string' && data.startsWith('data:')) {
        return data;
    }
    const base64 = typeof data === 'string' ? data : (0, base64_1.encodeUint8ArrayToBase64)(data);
    return mediaType ? `data:${mediaType};base64,${base64}` : base64;
}
//# sourceMappingURL=inlineData.js.map