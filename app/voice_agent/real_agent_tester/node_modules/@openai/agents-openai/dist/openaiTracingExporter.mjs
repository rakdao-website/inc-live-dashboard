                ...
                try {
                    ...
                    if (response.status >= 400 && response.status < 500) {
                        if (logger.dontLogModelData || logger.dontLogToolData) {
                            ...
                            logger.error(`[non-fatal] Tracing client error ${response.status}. Response data is redacted.`);
                        }
                        else {
                    ...
                    logModelAndToolActionError(logger, '[non-fatal] Tracing: request failed:', error);
                }
                if (signal?.aborted) {
